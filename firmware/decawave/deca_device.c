...
static inline void cs_select() {
    asm volatile("nop \n nop \n nop");
    gpio_put(UWB_PIN_CS, 0);  // Active low
    asm volatile("nop \n nop \n nop");
}
...
{
    cs_select();
    spi_write_blocking(UWB_SPI_PORT, headerBuffer, headerLength);
    spi_write_blocking(UWB_SPI_PORT, bodyBuffer, bodyLength);
    cs_deselect();
    return 0;
}
                ...
                uint8_t       * readBuffer)
{
    cs_select();
    spi_write_blocking(UWB_SPI_PORT, headerBuffer, headerLength);
    spi_read_blocking(UWB_SPI_PORT, 0, readBuffer, readLength);
    cs_deselect();
    return 0;
}
...
{   
    ...
    spi_write_blocking(UWB_SPI_PORT, tx_buf, len);
