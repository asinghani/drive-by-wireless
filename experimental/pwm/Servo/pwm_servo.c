...
bool direction = true;
int currentMillis = 1385;       //gives centre point (actual angle is 90deg from left wheel)
int servoPin = 22;
...
int main()
{
    ...
    while (true)
    {
        ...
        if (currentMillis >= 1685) direction = false;   //Right MAX from vehicles point of view
        if (currentMillis <= 1085) direction = true;    //Left MAX from vehicles point of view
        if (currentMillis <= 1085) direction = true;
